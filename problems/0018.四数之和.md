...
func fourSum(nums []int, target int) [][]int {
  ...
  for i := 0; i < len(nums) ; i++ {
    ...
    for j := i + 1; j < len(nums) ; j++ {
      ...
      targetSum := target - nums[i] - nums[j]  // 先算出双指针求和的目标值
      // 熟悉的双指针操作
      left := j + 1
      right := len(nums) - 1
      for left < right {
        sum := nums[left] + nums[right]
        if sum  == targetSum {  // 找到了，将四元组存下来
          for left < right && nums[left] == nums[left + 1] {  // 左指针去重
            left++
          }
          for left < right && nums[right] == nums[right - 1] {  // 右指针去重
            right--
          }
