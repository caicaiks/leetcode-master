...
func threeSum(s []int) [][]int {
  ...
  sort.Ints(s)
  for i := 0; i < len(s); i++ {
    if s[i] > 0 { // 最小的数大于0，则求和后无法得到0，直接排除
      continue
    }
    if i > 0 && s[i] == s[i-1] { // 最小数进行去重操作，直到找到跟上一个数不同的才开始计算
      continue
    }
    left := i + 1  // 左指针
    right := len(s) - 1  // 右指针
    for left < right {
      sum := s[i] + s[left] + s[right]   // 三元组求和，注意这里先不去重
      if sum == 0 { // 找到符合条件的三元组
        // 在这里去重
        ...
        // 将三元组append到结果中
        result = append(result, []int{s[i], s[left], s[right]})
        left++
        right--
        continue
      }
      if sum < 0 { // 如果求和小于0，说明left不够大
        left++
        continue
      }
      if sum > 0 { // 如果求和大于0，说明right太大
        right--
        continue
      }
    }
